...
def crop_recommend():
    title = 'Krushi - Crop Recommendation'
    return render_template('crop.html', title=title)

# render fertilizer recommendation form page
@app.route('/fertilizer')
def fertilizer_recommendation():
    title = 'Krushi - Fertilizer Suggestion'
...
@app.route('/app/templates/index2.html')
def index():
    context = {
        "top5": TopFiveWinners(),
...
def TopFiveWinners():
    ...
    for j in range(0, min(5, len(sorted_change))):  # Limit to top 5 if there are fewer items
        perc, i = sorted_change[j]
        crop_name = commodity_list[i].getCropName().split('/')[1]  # Extract crop name
        if crop_name in base:
            crop_price = round((current_month_prediction[i] * base[crop_name]) / 100, 2)
            to_send.append([crop_name, crop_price, round(perc, 2)])
        else:
            # Handle case where crop name is not found in the base dictionary
            # Log a warning or handle the error appropriately
            print(f"Base price not found for crop: {crop_name}")
